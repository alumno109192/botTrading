...
try:
    ...
    if turso_url and turso_token:
        ...
        db = DatabaseManager()
        print("✅ Sistema de tracking de BD activado (Silver 1D)")
    else:
...
SIMBOLOS = {
    'XAGUSD': {
        'ticker_yf':          'SI=F',       # Silver Futures en Yahoo Finance
        'zona_resist_high':   90.0,
        'zona_resist_low':    82.0,
        'zona_soporte_high':  73.0,        # Mínimo semana: 70.0
...
def analizar(simbolo, params):
    ...
    if aproximando_resistencia and not en_zona_resist and not cancelar_sell and not senal_buy_alerta and not ya_enviada('PREP_SELL'):
        msg = (f"🔔 <b>PREPARAR SELL LIMIT — PLATA (XAGUSD)</b> 🔔\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if aproximando_soporte and not en_zona_soporte and not cancelar_buy and not senal_sell_alerta and not ya_enviada('PREP_BUY'):
        msg = (f"🔔 <b>PREPARAR BUY LIMIT — PLATA (XAGUSD)</b> 🔔\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if cancelar_sell and not ya_enviada('CANCEL_SELL'):
        enviar_telegram(f"❌ <b>CANCELAR SELL LIMIT — PLATA (XAGUSD)</b> ❌\n━━━━━━━━━━━━━━━━━━━━\n"
                        f"💰 Precio: {fmt(close)} | Rompió resistencia ({fmt(zrh)})\n⏱️ 1D  📅 {fecha}")
        marcar_enviada('CANCEL_SELL')

    if cancelar_buy and not ya_enviada('CANCEL_BUY'):
        enviar_telegram(f"❌ <b>CANCELAR BUY LIMIT — PLATA (XAGUSD)</b> ❌\n━━━━━━━━━━━━━━━━━━━━\n"
                        f"💰 Precio: {fmt(close)} | Rompió soporte ({fmt(zsl)})\n⏱️ 1D  📅 {fecha}")
        marcar_enviada('CANCEL_BUY')

def main():
    print("🚀 Detector PLATA (XAGUSD) 1D iniciado")
    enviar_telegram("🚀 <b>Detector PLATA (XAGUSD) iniciado</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    while True:
        ciclo += 1
        print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 🔄 CICLO #{ciclo} - PLATA 1D")
