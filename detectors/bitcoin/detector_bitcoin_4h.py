...
CHECK_INTERVAL = 7 * 60  # cada 7 minutos (timeframe 4H requiere revisión más frecuente)

# ══════════════════════════════════════
# PARÁMETROS — ESPECÍFICOS BITCOIN 4H
# ══════════════════════════════════════
SIMBOLOS = {
    'BTCUSD': {
...
def analizar(simbolo, params):
    ...
    # ══════════════════════════════════
    senal_sell_maxima = score_sell >= 14  # Más estricto (era 10 en 1D)
    senal_sell_fuerte = score_sell >= 12  # Más estricto (era 8 en 1D)
    senal_sell_media  = score_sell >= 9   # Más estricto (era 6 en 1D)
    senal_sell_alerta = score_sell >= 5   # Más estricto (era 4 en 1D)
    ...
    if cancelar_sell and not ya_enviada('CANCEL_SELL'):
        msg = (f"❌ <b>CANCELAR SELL LIMIT — BITCOIN 4H</b> ❌\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
               f"💰 <b>Precio:</b> ${round(close, 0):,}\n"
    ...
    if cancelar_buy and not ya_enviada('CANCEL_BUY'):
        msg = (f"❌ <b>CANCELAR BUY LIMIT — BITCOIN 4H</b> ❌\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
               f"💰 <b>Precio:</b> ${round(close, 0):,}\n"
...
# ══════════════════════════════════════
def main():
    print("🚀 Detector BITCOIN 4H iniciado")
    print(f"⏱️  Revisando cada {CHECK_INTERVAL//60} minutos")

    enviar_telegram("🚀 <b>Detector BITCOIN 4H iniciado</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    while True:
        ...
        print(f"\n{'='*60}")
        print(f"[{ahora}] 🔄 CICLO #{ciclo} - Iniciando análisis BITCOIN 4H")
        print(f"{'='*60}")
