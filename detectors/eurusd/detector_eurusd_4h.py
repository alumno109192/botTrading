...
try:
    ...
    if turso_url and turso_token:
        ...
        db = DatabaseManager()
        print("✅ Sistema de tracking de BD activado (EURUSD 4H)")
    else:
...
SIMBOLOS = {
    'EURUSD': {
        'ticker_yf':          'EURUSD=X',
        'zona_resist_high':   1.2000,
        'zona_resist_low':    1.1850,
        'zona_soporte_high':  1.1600,
        'zona_soporte_low':   1.1400,
        'tp1_venta':          1.1650,
        'tp2_venta':          1.1450,
        'tp3_venta':          1.1200,
        'tp1_compra':         1.1900,
...
def detectar_evening_star(df, idx):
    ...
    return (v1['Close'] > v1['Open'] and
            abs(v1['Close']-v1['Open']) > (v1['High']-v1['Low'])*0.6 and
            abs(v2['Close']-v2['Open']) < (v2['High']-v2['Low'])*0.3 and
            v2['Open'] > v1['Close'] and
            v3['Close'] < v3['Open'] and
            abs(v3['Close']-v3['Open']) > (v3['High']-v3['Low'])*0.6 and
...
def calcular_sentimiento_bajista(row, prev, p2, df, params):
    ...
    if row['ema_fast'] < row['ema_slow']: factores.append("EMAs bajistas"); score += 1
    if close < row['ema_trend']: factores.append("Bajo EMA400"); score += 1
    if rsi > params['rsi_min_sell']: factores.append(f"RSI alto ({rsi:.1f})"); score += 1
    zrh = params['zona_resist_high']; zrl = params['zona_resist_low']; tol = params['tolerancia']
    if (high >= zrl - tol) and (high <= zrh + tol): factores.append("En resistencia"); score += 2
    try:
        lb = 5
        if len(df) >= lb + 3:
            if high > float(df['High'].iloc[-lb-2:-2].max()) and rsi < float(df['rsi'].iloc[-lb-2:-2].max()) and rsi > 50:
...
def calcular_sentimiento_alcista(row, prev, p2, df, params):
    ...
    if row['ema_fast'] > row['ema_slow']: factores.append("EMAs alcistas"); score += 1
    if close > row['ema_trend']: factores.append("Sobre EMA400"); score += 1
    if rsi < params['rsi_max_buy']: factores.append(f"RSI bajo ({rsi:.1f})"); score += 1
    zsh = params['zona_soporte_high']; zsl = params['zona_soporte_low']; tol = params['tolerancia']
    if (low >= zsl - tol) and (low <= zsh + tol): factores.append("En soporte"); score += 2
    try:
        lb = 5
        if len(df) >= lb + 3:
            if low < float(df['Low'].iloc[-lb-2:-2].min()) and rsi > float(df['rsi'].iloc[-lb-2:-2].min()) and rsi < 50:
...
def analizar(simbolo, params):
    ...
    try:
        ...
        if df.empty or len(df) < 200:
            print(f"⚠️ Datos insuficientes para {simbolo} 4H ({len(df) if not df.empty else 0} filas)")
            return
    except Exception as e:
    ...
    dist_to_resist = zrl - close; dist_to_support = close - zsh

    aproximando_resistencia = (dist_to_resist > 0 and dist_to_resist < avg_candle_range * av and close > df['Close'].iloc[-5])
    aproximando_soporte     = (dist_to_support > 0 and dist_to_support < avg_candle_range * av and close < df['Close'].iloc[-5])
    ...
    # Para forex EURUSD=X, el volumen puede ser 0 - usar seguro
    vol_avg_safe = vol_avg if (vol_avg and vol_avg > 0) else 1
    ...
    vol_alto_rechazo = vol > vol_avg_safe * vm
    ...
    score_sell = 0
    score_sell += 2 if en_zona_resist else 0; score_sell += 2 if vela_rechazo else 0
    score_sell += 2 if vol_alto_rechazo else 0; score_sell += 1 if rsi_alto_girando else 0
    score_sell += 1 if rsi_sobrecompra else 0; score_sell += 1 if divergencia_bajista else 0
    score_sell += 1 if emas_bajistas else 0; score_sell += 1 if estructura_bajista else 0
    score_sell += 1 if intento_rotura_fallido else 0; score_sell += 1 if vol_decreciente else 0
    score_sell += 1 if (shooting_star and vol_alto_rechazo) else 0
    score_sell += 1 if (divergencia_bajista and rsi_sobrecompra) else 0
    score_sell += 1 if bajo_ema200 else 0; score_sell += 2 if bb_toca_superior else 0
    score_sell += 2 if evening_star else 0; score_sell += 2 if macd_cruce_bajista else 0
    score_sell += 2 if adx_bajista else 0; score_sell += 1 if macd_divergencia_bajista else 0
    score_sell += 1 if obv_divergencia_bajista else 0; score_sell += 1 if obv_decreciente else 0
    ...
    vol_alto_rebote = vol > vol_avg_safe * vm
    ...
    score_buy = 0
    score_buy += 2 if en_zona_soporte else 0; score_buy += 2 if vela_rebote else 0
    score_buy += 2 if vol_alto_rebote else 0; score_buy += 1 if rsi_bajo_girando else 0
    score_buy += 1 if rsi_sobreventa else 0; score_buy += 1 if divergencia_alcista else 0
    score_buy += 1 if emas_alcistas else 0; score_buy += 1 if estructura_alcista else 0
    score_buy += 1 if intento_caida_fallido else 0; score_buy += 1 if vol_decreciente_sell else 0
    score_buy += 1 if (hammer and vol_alto_rebote) else 0
    score_buy += 1 if (divergencia_alcista and rsi_sobreventa) else 0
    score_buy += 1 if sobre_ema200 else 0; score_buy += 2 if bb_toca_inferior else 0
    score_buy += 2 if morning_star else 0; score_buy += 2 if macd_cruce_alcista else 0
    score_buy += 2 if adx_alcista else 0; score_buy += 1 if macd_divergencia_alcista else 0
    score_buy += 1 if obv_divergencia_alcista else 0; score_buy += 1 if obv_creciente else 0
    ...
    if aproximando_resistencia and not en_zona_resist and not cancelar_sell and not senal_buy_alerta and not ya_enviada('PREP_SELL'):
    ...
    if senal_sell_alerta and not cancelar_sell and rr_sell_tp1 >= 1.5:
        ...
        if not ya_enviada(tipo_clave):
            if db and db.existe_senal_reciente(simbolo_db, 'VENTA', horas=2):
                print("  ℹ️  Señal VENTA duplicada"); return
            msg = (f"{nivel} — <b>EURUSD (4H)</b>\n"
                   f"━━━━━━━━━━━━━━━━━━━━\n"
                   f"💰 <b>Precio:</b>     {fmt(close)}\n"
                   f"📌 <b>SELL LIMIT:</b> {fmt(sell_limit)}\n"
                   f"🛑 <b>Stop Loss:</b>  {fmt(sl_venta)}\n"
                   ...
                   f"🎯 <b>Calidad:</b> {calidad}  📉 RSI: {round(rsi,1)}\n"
                   f"⏱️ <b>TF:</b> 4H  📅 {fecha}")
            ...
            if db:
                try:
                    db.guardar_senal({'timestamp': datetime.now(timezone.utc), 'simbolo': simbolo_db, 'direccion': 'VENTA',
                        ...
                        'indicadores': json.dumps({'rsi': round(rsi, 1), 'atr': round(atr, 5), 'adx': round(adx, 2)}),
                        'patron_velas': f"Evening Star: {evening_star}", 'version_detector': '3.0'})
    ...
    if senal_buy_alerta and not cancelar_buy and rr_buy_tp1 >= 1.5:
        ...
        if not ya_enviada(tipo_clave):
            if db and db.existe_senal_reciente(simbolo_db, 'COMPRA', horas=2):
                print("  ℹ️  Señal COMPRA duplicada"); return
            msg = (f"{nivel} — <b>EURUSD (4H)</b>\n"
                   f"━━━━━━━━━━━━━━━━━━━━\n"
                   f"💰 <b>Precio:</b>    {fmt(close)}\n"
                   f"📌 <b>BUY LIMIT:</b> {fmt(buy_limit)}\n"
            ...
            if db:
                try:
                    db.guardar_senal({'timestamp': datetime.now(timezone.utc), 'simbolo': simbolo_db, 'direccion': 'COMPRA',
                        ...
                        'indicadores': json.dumps({'rsi': round(rsi, 1), 'atr': round(atr, 5), 'adx': round(adx, 2)}),
                        'patron_velas': f"Morning Star: {morning_star}", 'version_detector': '3.0'})
                except Exception as e: print(f"  ⚠️ Error BD: {e}")
            enviar_telegram(msg); marcar_enviada(tipo_clave)

    if cancelar_sell and not ya_enviada('CANCEL_SELL'):
        enviar_telegram(f"❌ <b>CANCELAR SELL LIMIT — EURUSD (4H)</b> ❌\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if cancelar_buy and not ya_enviada('CANCEL_BUY'):
        enviar_telegram(f"❌ <b>CANCELAR BUY LIMIT — EURUSD (4H)</b> ❌\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def main():
    print("🚀 Detector EURUSD 4H iniciado")
    enviar_telegram("🚀 <b>Detector EURUSD 4H iniciado</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
                    "📊 Monitorizando: EURUSD=X\n"
                    "⏱️ Timeframe: 4H\n"
                    f"🔄 Revisión cada {CHECK_INTERVAL//60} minutos\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
                    "🔴 Resistencia: 1.1050 - 1.1200\n"
                    "🟢 Soporte:     1.0600 - 1.0750")
    ciclo = 0
    while True:
        ciclo += 1
        print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 🔄 CICLO #{ciclo} - EURUSD 4H")
