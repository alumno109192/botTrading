"""
DETECTOR EUR/USD 1H - INTRADIARIO
Análisis de EURUSD en timeframe 1 hora para operaciones intradía.
Resamplea velas 5M a 1H. Sesión activa: 07:00-21:00 UTC.
"""
import os
import json
import time
...
CHECK_INTERVAL     = 60   # 1 minuto entre análisis

# ══════════════════════════════════════
# PARÁMETROS — INTRADAY EUR/USD 1H
# ══════════════════════════════════════
SIMBOLOS = {
    'EURUSD': {
        'ticker_yf':          'EURUSD=X',
        ...
        'atr_tp2_mult':       2.5,
        'atr_tp3_mult':       3.5,
        'atr_min':            0.0005,     # ATR mínimo: 5 pips
...
class EURUSDDetector1H(BaseDetector):
    def analizar(self, simbolo, params):

        # ── Bloqueo por eventos críticos ──
        bloqueado, desc_evento, minutos = debe_bloquear_trading(120)
        if bloqueado:
            logger.warning(f"  🚫 [EURUSD 1H] Trading bloqueado — {desc_evento} en {minutos} min")
            enviar_alerta_bloqueo(desc_evento, minutos, ['EURUSD 1H'])
            return

        self.aviso_macro = obtener_aviso_macro(60, '1H', simbolo)

        try:
            # Obtener datos de período más largo para tener suficientes velas 1H
            df_5m, _ = get_ohlcv(params['ticker_yf'], period='60d', interval='5m')
            if df_5m is None or df_5m.empty or len(df_5m) < 300:
                # Intentar con intervalo nativo 1H si 5M falla
                df, _ = get_ohlcv(params['ticker_yf'], period='60d', interval='1h')
                if df is None or df.empty or len(df) < 50:
                    logger.warning(f"⚠️ Datos insuficientes para {simbolo} EURUSD 1H")
                    return
            else:
            ...
            if len(df) < 50:
                logger.warning(f"⚠️ Pocos datos tras resample EURUSD 1H")
                return
            ...
            # EMA 200 como filtro de tendencia macro
            ...
            _cuña_desc, _, _ = detectar_cuña_descendente(df, atr, lookback=_lkb, wing=2, max_amplitud_pct=0.02)
            _cuña_asc, _, _  = detectar_cuña_ascendente(df, atr, lookback=_lkb, wing=2, max_amplitud_pct=0.02)
            ...
            logger.info(f"  [EURUSD 1H] {fecha}  Close:{close:.5f}  SELL:{score_sell}  BUY:{score_buy}  RSI:{rsi:.1f}  ADX:{adx:.1f}  ATR:{atr:.5f}")
            ...
            if senal_sell_fuerte and _sh_alc_activo:
                logger.warning(f"  🚫 [ANTI-TRAMPA EURUSD 1H] SELL bloqueada: Stop Hunt ALCISTA activo")
                senal_sell_fuerte = False
            if senal_buy_fuerte and _sh_baj_activo:
                logger.warning(f"  🚫 [ANTI-TRAMPA EURUSD 1H] BUY bloqueada: Stop Hunt BAJISTA activo")
                senal_buy_fuerte = False
            _warn_consenso_sell = tf_bias.detectar_consenso_trampa(simbolo, '1H', tf_bias.BIAS_BEARISH)
            _warn_consenso_buy  = tf_bias.detectar_consenso_trampa(simbolo, '1H', tf_bias.BIAS_BULLISH)
            ...
            if senal_sell_fuerte and not cancelar_sell:
                if self.db and self.db.existe_senal_activa_tf(simbolo_db):
                    logger.info(f"  ℹ️  SELL 1H bloqueada — señal activa en {simbolo_db}")
                else:
                    ...
                    msg = (
                        ...
                        f"⏱️ <b>TF:</b> 1H  🕐 {hora_envio}\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"🔍 <b>Patrones:</b>\n{diag}\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"⏳ INTRADAY — Cerrar al cierre de sesión"
                    )
                    if _conf_sell:
                        msg += f"\n━━━━━━━━━━━━━━━━━━━━\n{_conf_sell}"
                    ...
                    if self.db:
                        try:
                            _sid = self._guardar_senal({
                                ...
                                'simbolo': simbolo_db, 'asset': 'EURUSD', 'timeframe': '1H',
                                ...
                                'version_detector': 'EURUSD-1H-v1.0',
                            })
                            if _sid is None:
                                logger.warning(f"  🔴 SELL bloqueada por guard interno — Telegram no enviado")
                            _senal_guardada = _sid is not None
                        except Exception as e:
                            logger.error(f"  ⚠️ Error guardando señal EURUSD 1H SELL: {e}")
                    if _senal_guardada:
                        self.enviar(msg)

            if senal_buy_fuerte and not cancelar_buy:
                if self.db and self.db.existe_senal_activa_tf(simbolo_db):
                    logger.info(f"  ℹ️  BUY 1H bloqueada — señal activa en {simbolo_db}")
                else:
                    hora_envio = datetime.now(timezone.utc).strftime('%H:%M:%S UTC')
                    msg = (
                    ...
                    if self.db:
                        try:
                            ...
                            logger.error(f"  ⚠️ Error guardando señal EURUSD 1H BUY: {e}")
                    if _senal_guardada:
                        self.enviar(msg)

        except Exception as e:
            logger.error(f"❌ Error analizando {simbolo} [EURUSD 1H]: {e}", exc_info=True)


def analizar_simbolo(simbolo, params):
    return EURUSDDetector1H(
        simbolo=simbolo, tf_label='1H',
...
def main():
    ...
    _tg("🚀 <b>Detector EUR/USD 1H INTRADAY iniciado</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "⏱️ Análisis cada minuto\n"
    ...
    while True:
        ...
        if ahora_utc.weekday() == 5:
            logger.info(f"[EURUSD 1H] 💤 Sábado — mercado cerrado")
            time.sleep(3600)
            continue
        logger.info(f"[{ahora_utc.strftime('%H:%M:%S')}] 🔄 EURUSD 1H — ciclo #{ciclo}")
