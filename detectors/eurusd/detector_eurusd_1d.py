...
try:
    ...
    if turso_url and turso_token:
        ...
        db = DatabaseManager()
        print("✅ Sistema de tracking de BD activado (EURUSD 1D)")
    else:
...
SIMBOLOS = {
    'EURUSD': {
        ...
        'zona_resist_high':   1.2000,       # Máximo 1Y: 1.2024
        'zona_resist_low':    1.1850,
        'zona_soporte_high':  1.1600,       # Mínimo semana: 1.1507
        ...
        'tp2_compra':         1.2000,
        'tp3_compra':         1.2100,
        'tolerancia':         0.0040,       # 40 pips
        'limit_offset_pct':   0.3,
        'anticipar_velas':    3,
        'cancelar_dist':      0.5,          # 0.5% de cancelación
        'rsi_length':         14,
        'rsi_min_sell':       60.0,
        'rsi_max_buy':        40.0,
        'ema_fast_len':       9,
...
def calcular_sentimiento_bajista(row, prev, p2, df, params):
    ...
    if close < ema_trend:
        factores.append("Precio bajo EMA200"); score += 1
...
def calcular_sentimiento_alcista(row, prev, p2, df, params):
    ...
    if close > ema_trend:
        factores.append("Precio sobre EMA200"); score += 1
...
def analizar(simbolo, params):
    ...
    bb_mid    = row['bb_mid'];   bb_width = row['bb_width']
    ...
    obv       = row['obv'];      obv_prev = prev['obv']
    obv_ema   = row['obv_ema']
    adx       = row['adx'];      di_plus = row['di_plus']; di_minus = row['di_minus']

    body        = row['body'];   upper_wick = row['upper_wick']
    lower_wick  = row['lower_wick']; total_range = row['total_range']
    is_bearish  = row['is_bearish']; is_bullish = row['is_bullish']

    zrh = params['zona_resist_high']; zrl = params['zona_resist_low']
    zsh = params['zona_soporte_high']; zsl = params['zona_soporte_low']
    tol = params['tolerancia'];  lop = params['limit_offset_pct']
    cd  = params['cancelar_dist']; av = params['anticipar_velas']
    vm  = params['vol_mult'];    rsms = params['rsi_min_sell']
    rsmb = params['rsi_max_buy']; asm = params['atr_sl_mult']
    ...
    # Para forex EURUSD=X, el volumen es tick count - puede ser 0 en algunos períodos
    ...
    # Formato de precio para EURUSD (4 decimales, sin símbolo $)
    def fmt(v):
        return f"{v:.4f}"
        ...
        if not ya_enviada(tipo_clave):
            ...
            if db:
                senal_data = {
                    ...
                    'simbolo': simbolo, 'direccion': 'VENTA',
                    'precio_entrada': sell_limit,
                    'tp1': tp1_v, 'tp2': tp2_v, 'tp3': tp3_v, 'sl': sl_venta, 'score': score_sell,
                    'indicadores': json.dumps({'rsi': round(rsi, 1), 'atr': round(atr, 5), 'adx': round(adx, 2), 'macd': round(macd, 6)}),
    ...
    if senal_buy_alerta and not cancelar_buy:
        ...
        if not ya_enviada(tipo_clave):
            ...
            if db:
                senal_data = {
                    'timestamp': datetime.now(timezone.utc),
                    'simbolo': simbolo, 'direccion': 'COMPRA',
                    'precio_entrada': buy_limit,
                    'tp1': tp1_c, 'tp2': tp2_c, 'tp3': tp3_c, 'sl': sl_compra, 'score': score_buy,
                    'indicadores': json.dumps({'rsi': round(rsi, 1), 'atr': round(atr, 5), 'adx': round(adx, 2), 'macd': round(macd, 6)}),
    ...
    if cancelar_sell and not ya_enviada('CANCEL_SELL'):
        msg = (f"❌ <b>CANCELAR SELL LIMIT — EURUSD</b> ❌\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
               f"💰 <b>Precio:</b> {fmt(close)}\n"
    ...
    if cancelar_buy and not ya_enviada('CANCEL_BUY'):
        msg = (f"❌ <b>CANCELAR BUY LIMIT — EURUSD</b> ❌\n"
               f"━━━━━━━━━━━━━━━━━━━━\n"
               f"💰 <b>Precio:</b> {fmt(close)}\n"
...
def main():
    print("🚀 Detector EURUSD 1D iniciado")

    enviar_telegram("🚀 <b>Detector EURUSD iniciado</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    while True:
        ...
        print(f"\n{'='*60}")
        print(f"[{ahora}] 🔄 CICLO #{ciclo} - Iniciando análisis EURUSD")
        print(f"{'='*60}")
