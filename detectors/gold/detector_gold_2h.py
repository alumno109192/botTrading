...
SIMBOLOS = {
    'XAUUSD': {
        ...
        'sr_lookback':        100,          # 100 velas 2H ≈ 8 días de historia
        ...
        'rsi_max_buy':        45.0,
        'ema_fast_len':       12,      # Intermedio 2H
        'ema_slow_len':       28,
        'ema_trend_len':      280,
        'atr_length':         14,
        'atr_sl_mult':        1.1,    # SL 2H
        'atr_tp1_mult':       1.8,    # TP1: 1.8× ATR (intradía alcanzable)
        'atr_tp2_mult':       3.0,    # TP2: 3.0× ATR
        'atr_tp3_mult':       4.5,    # TP3: 4.5× ATR (objetivo ambicioso)
        'vol_mult':           1.2,
...
_detector_instance: 'GoldDetector2H | None' = None
_predictor_2h_buy = GoldPredictor(tf='2H', direccion='COMPRA')
_predictor_2h_sell = GoldPredictor(tf='2H', direccion='VENTA')
_last_ml_retrain_2h = 0.0
...
def analizar(simbolo, params):
    ...
    if _detector_instance is None:
        _detector_instance = GoldDetector2H(
            simbolo=simbolo, tf_label='2H', params=params,
...
class GoldDetector2H(BaseDetector):
    def analizar(self, simbolo, params):
        simbolo_db = f"{simbolo}_2H"

        # ── Bloqueo por eventos críticos (FOMC, Powell, NFP, CPI) ──
        bloqueado, desc_evento, minutos = debe_bloquear_trading(90)
        if bloqueado:
            enviar_alerta_bloqueo(desc_evento, minutos, ['2H'])
            logger.warning(f"🚫 [2H] Trading bloqueado por evento: {desc_evento}")
            return
        
        # ── Aviso calendario económico (eventos menores) ──
        self.aviso_macro = obtener_aviso_macro(60, '2H', simbolo)

        logger.info(f"\n🔍 Analizando {simbolo} [2H intradía]...")

        try:
            # NOTA: TwelveData tiene problemas con el endpoint 2H directo
            # Solución: descargar 5M (confiable) y resamplear a 2H
            df_5m, is_delayed = get_ohlcv(params['ticker_yf'], period='7d', interval='5m')
            if df_5m.empty or len(df_5m) < 100:
                logger.warning(f"⚠️ Datos insuficientes para {simbolo} 2H")
                return
            
            # Resample 5M → 2H (más confiable que endpoint 2H directo de TD)
            df = df_5m.resample('2h').agg({
        ...
        if _atr_raw > 160:
            logger.warning(f"  ⚠️ [2H] ATR anómalo ({_atr_raw:.1f}) — posible contaminación de datos, saltando ciclo")
            logger.warning(f"  🧹 [2H] Limpiando cache memoria y BD contaminada...")
            ...
            with _intraday_cache_lock:
                ...
                logger.warning(f"  🗑️ [2H] Cache memoria limpiado ({len(claves)} entradas)")
            ...
            try:
                ...
                logger.warning(f"  🗑️ [2H] BD limpiada — próximo ciclo descargará datos frescos")
            except Exception as e:
                logger.error(f"  ⚠️ [2H] Error limpiando BD: {e}")
        ...
        # ±1.0: offset para asegurar fill — SELL $1 por debajo, BUY $1 por encima del nivel calculado
        sell_entry = round(sell_limit - spread - 1.0, 2)
        buy_entry  = round(buy_limit  + spread + 1.0, 2)
        ...
        if self.db and self.db.existe_senal_activa_misma_dir(simbolo_db, 'VENTA'):
            cancelar_sell = True
            logger.info(f"  🚫 [2H] cancelar_sell=True: VENTA ya activa en BD para {simbolo_db}")
        if self.db and self.db.existe_senal_activa_misma_dir(simbolo_db, 'COMPRA'):
            cancelar_buy = True
            logger.info(f"  🚫 [2H] cancelar_buy=True: COMPRA ya activa en BD para {simbolo_db}")
        # Bloquear si hay señal ACTIVA en dirección contraria
        if self.db and self.db.existe_senal_activa_opuesta(simbolo_db, 'VENTA'):
            cancelar_buy = True
            logger.info(f"  🚫 [2H] cancelar_buy=True: VENTA activa en BD para {simbolo_db}")
        ...
        _etiq_sell = 'NEUTRO'
        try:
            _pred_features = _predictor_2h_buy.calcular_features_predictivos(
                df, zsl, zsh, zrl, zrh, atr
            )
        except Exception as e:
            logger.debug(f"  [ML 1H] Features predictivos no disponibles: {e}")
        try:
            _prob_buy, _etiq_buy = _predictor_2h_buy.predecir(_pred_features)
        except Exception as e:
            logger.debug(f"  [ML 1H] Predicción BUY no disponible: {e}")
            _prob_buy = 0.0
            _etiq_buy = 'NEUTRO'
        try:
            _prob_sell, _etiq_sell = _predictor_2h_sell.predecir(_pred_features)
        except Exception as e:
        ...
        score_sell = (
            ...
            (1 if vol_alto_rechazo   else 0)     # volumen confirma movimiento bajista
        )
        # BONUS SELL — confirmaciones adicionales, cada una vale +1, cap duro en 4
        _bonus_sell = (
            (1 if rsi_alto_girando     else 0) +
            (1 if vela_rechazo         else 0) +
            (1 if macd_cruce_bajista   else 0) +
            (1 if adx_bajista          else 0) +
            (1 if divergencia_bajista  else 0) +   # sin doble conteo con rsi_sobrecompra
            (1 if evening_star         else 0) +
            (1 if canal_alcista_roto   else 0)
        )
        ...
        if _rup_sop_1h:
            _bonus_sell += 1
            logger.info(f"  💥 [2H] RUPTURA SOPORTE ${_niv_sop_1h:.2f} — +1 pt SELL")
        if _rup_res_1h:
            _bonus_buy += 1
            logger.info(f"  💥 [2H] RUPTURA RESISTENCIA ${_niv_res_1h:.2f} — +1 pt BUY")
        ...
        if _en_resist_sr_1h and not en_zona_resist:
            ...
            _bonus_sell += 1
            logger.info(f"  📌 [2H] Precio en resistencia S/R intermedia ${_nsr:.2f} — +1 pt SELL")
        if _en_sop_sr_1h and not en_zona_soporte:
            _nsr2 = min(_sop_interm_1h, key=lambda s: abs(low - s))
            _bonus_buy += 1
            logger.info(f"  📌 [2H] Precio en soporte S/R intermedio ${_nsr2:.2f} — +1 pt BUY")
        ...
        if _cuña_desc_1h == 'ruptura_alcista':
            _bonus_buy += 1
            logger.info(f"  📐 [2H] CUÑA DESC ROTA AL ALZA (techo ${_t_desc_1h:.2f}) — +1 pt BUY")
        elif _cuña_desc_1h == 'ruptura_bajista':
            _bonus_sell += 1
            logger.info(f"  📐 [2H] CUÑA DESC ROTA A LA BAJA (suelo ${_s_desc_1h:.2f}) — +1 pt SELL")
        if _cuña_asc_1h == 'ruptura_bajista':
            _bonus_sell += 1
            logger.info(f"  📐 [2H] CUÑA ASC ROTA A LA BAJA (suelo ${_s_asc_1h:.2f}) — +1 pt SELL")
        ...
        if _dt_1h:
            _bonus_sell += 1
            logger.info(f"  🔻 [2H] DOBLE TECHO (M) detectado — techo=${_dt_nivel_1h:.1f} cuello=${_dt_neck_1h:.1f} — +1 pt SELL")
        if _ds_1h:
            _bonus_buy += 1
            logger.info(f"  🔺 [2H] DOBLE SUELO (W) detectado — suelo=${_ds_nivel_1h:.1f} cuello=${_ds_neck_1h:.1f} — +1 pt BUY")
        ...
        if v_rev_alc_1h:
            _bonus_buy += 1
            logger.info(f"  ⚡ [2H] V-REVERSAL ALCISTA detectado — mín ${v_min_1h:.2f} → ${v_precio_1h:.2f} — +1 pt BUY")
        if v_rev_baj_1h:
            _bonus_sell += 1
            logger.info(f"  ⚡ [2H] V-REVERSAL BAJISTA detectado — máx ${v_max_1h:.2f} → ${v_precio_baj_1h:.2f} — +1 pt SELL")
        ...
        score_buy += (
            ...
            (1 if vol_alto_rebote     else 0)     # volumen confirma movimiento alcista
        )
        # BONUS BUY — confirmaciones adicionales, cada una vale +1
        _bonus_buy += (
            (1 if rsi_bajo_girando         else 0) +
            (1 if vela_rebote              else 0) +
            (1 if macd_cruce_alcista       else 0) +
            (1 if adx_alcista              else 0) +
            (1 if divergencia_alcista      else 0) +   # sin doble conteo con rsi_sobreventa
            (1 if morning_star             else 0) +
            (1 if canal_bajista_roto       else 0)
        )
        ...
        if _fib_nivel_1h is not None:
            if _fib_tend_1h == 'bajista':
                _bonus_sell += 1
                logger.info(f"  🔢 [2H] Fib {_fib_nivel_1h} BAJISTA en ${_fib_precio_1h:.2f} — +1 pt SELL")
            else:
                _bonus_buy += 1
                logger.info(f"  🔢 [2H] Fib {_fib_nivel_1h} ALCISTA en ${_fib_precio_1h:.2f} — +1 pt BUY")

        _fib_data_1h = calcular_fibonacci(df, lookback=params.get('sr_lookback', 80))
        if _fib_data_1h:
            _n1 = _fib_data_1h['niveles']
            logger.info(
                f"  📐 [2H] Fib ({_fib_data_1h['tendencia']}) "
        ...
        if _pivots_1h:
            ...
            logger.info(f"  🎯 [2H] Pivots diarios: PP=${_pivots_1h['PP']:.2f} | {_piv_info}")
        ...
        if _rebote_baj_1h:
            _bonus_sell += 1
            logger.info(f"  🔄 [2H] REBOTE BAJISTA detectado ({_desc_baj_1h}) — +1 pt SELL")
        if _rebote_alc_1h:
            _bonus_buy += 1
            logger.info(f"  🔄 [2H] REBOTE ALCISTA detectado ({_desc_alc_1h}) — +1 pt BUY")

        # ── Aplicar cap duro de bonus (máx 4 pts) ──────────────────────────────
        score_sell += min(_bonus_sell, 4)
        score_buy  += min(_bonus_buy,  4)
        ...
        if _vol_bajo:
            logger.info(f"  ⚠️ [2H] Volumen bajo ({vol:.0f} < {vol_avg * vm:.0f}) — scores penalizados -3")
        ...
        score_sell = min(score_sell + _n_confirm_sell, 10)
        score_buy  = min(score_buy  + _n_confirm_buy,  10)
...
"""
        ...
        if _micro_vol > 1.5:
            if score_sell > score_buy:
                score_sell = min(score_sell + 1, 10)
                logger.info(f"  📈 [2H] Micro-vol {_micro_vol:.2f} (expansión) — +1 SELL")
            elif score_buy > score_sell:
                score_buy = min(score_buy + 1, 10)
                logger.info(f"  📈 [2H] Micro-vol {_micro_vol:.2f} (expansión) — +1 BUY")
        elif _micro_vol < 0.8:
            score_sell = max(0, score_sell - 1)
            score_buy  = max(0, score_buy  - 1)
            logger.info(f"  😴 [2H] Micro-vol {_micro_vol:.2f} (dormido) — -1 ambos scores")
        if _momentum_rec == -1 and en_zona_resist_any:
            score_sell = min(score_sell + 1, 10)
            logger.info(f"  🔻 [2H] Momentum bajista en resistencia — +1 SELL")
        elif _momentum_rec == 1 and en_zona_soporte_any:
            score_buy = min(score_buy + 1, 10)
            logger.info(f"  🔺 [2H] Momentum alcista en soporte — +1 BUY")

        # ── Ajuste de score por predicción anticipada ML — cap global /10 ───
        if _prob_buy > 0.65:
            score_buy = min(score_buy + 1, 10)
            logger.info(f"  🤖 [ML 2H] Probabilidad BUY ({_prob_buy:.1%}) — +1 pt BUY")
        if _prob_sell > 0.65:
            score_sell = min(score_sell + 1, 10)
            logger.info(f"  🤖 [ML 2H] Probabilidad SELL ({_prob_sell:.1%}) — +1 pt SELL")
        ...
        _umbral_max = self.umbral_adaptativo(9, atr, atr_media)   # core 6 + bonus cap 4 = máx 10
        _umbral_fue = self.umbral_adaptativo(7, atr, atr_media)
        _umbral_med = self.umbral_adaptativo(5, atr, atr_media)
        _umbral_ale = self.umbral_adaptativo(4, atr, atr_media)
        ...
        _ADX_MIN = 15
        if adx < _ADX_MIN:
            logger.info(f"  😴 [2H] ADX {round(adx, 1)} < {_ADX_MIN} — mercado plano, todas las señales bloqueadas")
        ...
        if not self.en_sesion_optima():
            if senal_sell_alerta and not senal_sell_media:
                logger.info(f"  🌙 [2H] Fuera sesión óptima — SELL ALERTA suprimida")
            if senal_buy_alerta and not senal_buy_media:
                logger.info(f"  🌙 [2H] Fuera sesión óptima — BUY ALERTA suprimida")
        ...
        logger.info(f"  📅 {fecha} | Close: ${close:.2f} | ATR: ${atr:.2f} | SELL: {score_sell}/10 | BUY: {score_buy}/10")
        ...
        _clave_pred = f"{simbolo}_2H_PRED_{fecha}"
            ...
            if self.db:
                try:
                    self._guardar_senal({
                        ...
                        'version_detector': 'GOLD 2H-ML-v1.0',
        ...
        if _prep_buy_alerta and not cancelar_buy and not cancelar_buy_rr:
            ...
            logger.info(f"  🏗️ [2H] Zona BUY activa publicada — zona ${zsl:.2f}-${zsh:.2f}, limit ${buy_limit:.2f}")
        elif cancelar_buy:
        ...
        if _prep_sell_alerta and not cancelar_sell and not cancelar_sell_rr:
            ...
            logger.info(f"  🏗️ [2H] Zona SELL activa publicada — zona ${zrl:.2f}-${zrh:.2f}, limit ${sell_limit:.2f}")
        elif cancelar_sell:
        ...
        if senal_sell_media:
            ...
            if not _ok:
                ...
                _conf_sell = _desc
        if senal_buy_media:
            _ok, _desc = tf_bias.verificar_confluencia(simbolo, '2H', tf_bias.BIAS_BULLISH, score=score_buy)
            if not _ok:
        ...
        try:
            ...
            logger.debug(f"  [2H] Error _ctx_htf: {_ctx_e}")
        # ─────────────────────────────────────────────────────────────────────
        ...
        if aproximando_resist and not en_zona_resist and not cancelar_sell and not cancelar_sell_rr and _prep_sell_alerta and not self.ya_enviada(f"{clave_vela}_PREP_SELL"):
            ...
            if _bloquear_prep_sell:
                ...
                if self._debe_suprimir_por_evento(_nivel_prep_sell):
                    logger.info(f"  🔕 [2H] SELL PREP suprimida por evento macro")
                else:
                    if _ctx_htf:
                        msg += _ctx_htf
        ...
        if aproximando_soporte and not en_zona_soporte and not cancelar_buy and not cancelar_buy_rr and _prep_buy_alerta and not self.ya_enviada(f"{clave_vela}_PREP_BUY"):
            ...
            if _bloquear_prep_buy:
                ...
                if self._debe_suprimir_por_evento(_nivel_prep_buy):
                    logger.info(f"  🔕 [2H] BUY PREP suprimida por evento macro")
                else:
                    if _ctx_htf:
                        msg += _ctx_htf
                ...
                if not self.ya_enviada(f"{clave_vela}_{tipo_clave}"):
                    if self.ya_enviada(f"{clave_vela}_PREP_SELL"):
                        ...
                        msg = (f"✅ <b>CONFIRMACIÓN SELL — {self.nombre_display} {self.tf_label}</b>\n"
                               ...
                               f"📊 <b>Score:</b> {score_sell}/10  📉 <b>RSI:</b> {round(rsi, 1)}\n"
                               f"⏱️ <b>TF:</b> 2H  📅 {fecha}")
                ...
                if not self.ya_enviada(f"{clave_vela}_{tipo_clave}"):
                    if self.ya_enviada(f"{clave_vela}_PREP_BUY"):
                        ...
                        msg = (f"✅ <b>CONFIRMACIÓN BUY — {self.nombre_display} {self.tf_label}</b>\n"
                               ...
                               f"{nivel} — precio ahora en zona\n"
                               f"💰 <b>Precio:</b> ${close:.2f}\n"
                               f"📊 <b>Score:</b> {score_buy}/10  📉 <b>RSI:</b> {round(rsi, 1)}\n"
                               f"⏱️ <b>TF:</b> 2H  📅 {fecha}")
            ...
            if pullback_alcista:
                ...
                if rr1 >= 1.5 and score_sell >= 2:   # R:R mínimo + al menos 2 confirmaciones técnicas
                    ...
                    msg = (
                        ...
                        f"📐 <b>ATR:</b> ${atr:.2f}  📊 <b>Score:</b> {score_sell}/10\n"
                        f"⏱️ 2H  📅 {fecha}  🏆 Setup retest canal"
                    )
                    if self.db:
                        try:
                            self._guardar_senal({
                                ...
                                'version_detector': 'GOLD 2H-v1.0-RETEST'
                            })
                        except Exception as e:
                            logger.error(f"  ⚠️ Error BD: {e}")
                    self.enviar(msg)
            ...
            if pullback_bajista:
                ...
                if rr1 >= 1.5 and score_buy >= 2:   # R:R mínimo + al menos 2 confirmaciones técnicas
                    ...
                    msg = (
                        ...
                        f"📐 <b>ATR:</b> ${atr:.2f}  📊 <b>Score:</b> {score_buy}/10\n"
                        f"⏱️ 2H  📅 {fecha}  🏆 Setup retest canal"
                    )
                    if self.db:
                        try:
...
def main():
    global _last_ml_retrain_2h
    logger.info("🚀 Detector ORO 2H intradía iniciado")
    enviar_telegram(f"🚀 <b>Detector {simbolo_a_nombre('XAUUSD')} 2H — INTRADÍA iniciado</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
                    "📊 Monitorizando: GC=F (Gold Futures)\n"
                    "⏱️ Timeframe: 2H  |  Modo: ⏰ INTRADÍA\n"
                    "🔄 Revisión cada 60 segundos\n"
                    "📐 TPs dinámicos basados en ATR (×1.8 / ×3.0 / ×4.5)\n"
    ...
    while True:
        ...
        logger.info(f"\n[{ahora_utc.strftime('%Y-%m-%d %H:%M:%S')}] 🔄 CICLO #{ciclo} - ORO 2H")
        if (_predictor_2h_buy.necesita_reentrenamiento() or
                _predictor_2h_sell.necesita_reentrenamiento() or
                (time.time() - _last_ml_retrain_2h) >= 7 * 24 * 3600):
            try:
                _predictor_2h_buy.reentrenar_desde_bd(db)
                _predictor_2h_sell.reentrenar_desde_bd(db)
                _last_ml_retrain_2h = time.time()
                logger.info("  ✅ [ML] Modelos 2H re-entrenados")
            except Exception as e:
