        ...
        try:
            ...
            if _micro_vol > 1.5:
                if score_sell > score_buy:
                    ...
                    logger.info(f"  📈 [5M] Micro-vol {_micro_vol:.2f} (expansión) — +1 SELL")
                elif score_buy > score_sell:
                    score_buy = min(score_buy + 1, 23)
                    logger.info(f"  📈 [5M] Micro-vol {_micro_vol:.2f} (expansión) — +1 BUY")
            elif _micro_vol < 0.8:
                score_sell = max(0, score_sell - 1)
                score_buy  = max(0, score_buy  - 1)
                logger.info(f"  😴 [5M] Micro-vol {_micro_vol:.2f} (dormido) — -1 ambos scores")
            if _momentum_rec == -1 and (en_zona_resist or aproximando_resist):
                score_sell = min(score_sell + 1, 23)
                logger.info(f"  🔻 [5M] Momentum bajista en resistencia — +1 SELL")
            ...
            tp1_v = round(sell_limit - atr_efectivo * params['atr_tp1_mult'] * _vol_factor, 2)
            tp2_v = round(sell_limit - atr_efectivo * params['atr_tp2_mult'] * _vol_factor, 2)
            tp3_v = round(sell_limit - atr_efectivo * params['atr_tp3_mult'] * _vol_factor, 2)
            tp1_c = round(buy_limit  + atr_efectivo * params['atr_tp1_mult'] * _vol_factor, 2)
            tp2_c = round(buy_limit  + atr_efectivo * params['atr_tp2_mult'] * _vol_factor, 2)
            tp3_c = round(buy_limit  + atr_efectivo * params['atr_tp3_mult'] * _vol_factor, 2)
